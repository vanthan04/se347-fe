...
const CustomerBookNow = () => {
  ...
  useEffect(() => {
    ...
    const handleOrderUpdate = () => {
      // Reload booking draft to get updated tasker_id
      const draft = localStorage.getItem("bookingDraft");
      if (draft) {
        const parsedDraft = JSON.parse(draft);
        setBookingDraft(parsedDraft);

        // Load tasker info if tasker_id is now available
        if (parsedDraft.tasker_id && !taskerInfo) {
          loadTaskerInfo(parsedDraft.tasker_id);
        }
      }
    };

    window.addEventListener("ORDER_ACCEPTED", handleOrderUpdate);

    return () => {
      window.removeEventListener("ORDER_ACCEPTED", handleOrderUpdate);
    };
  }, [taskerInfo]);
  ...
  const loadTaskerInfo = async (taskerId) => {
    try {
      if (!taskerId) return;
      ...
      console.error("Error loading tasker info:", err);
    }
  };

  const loadTaskerAndTaskInfo = async (taskerId, taskId) => {
    try {
      // Load tasker info only if taskerId exists
      if (taskerId) {
        await loadTaskerInfo(taskerId);
      }
        ...
        {taskerInfo ? (
          ...
          <div className="mb-3 pb-3 border-b">
            ...
            <div className="flex items-center gap-2 text-gray-500">
              ...
              <p className="text-sm italic">Đang tìm tasker phù hợp...</p>
            </div>
          </div>
        )}
